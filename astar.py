...
def astar_node_count(blocked: set, start: tuple, goal: tuple,
                     cols: int, rows: int) -> tuple:
    """
    Versi perluasan dari astar() yang mengembalikan jumlah node
    yang diperluas — berguna untuk perbandingan akademik dengan BFS.

    Kembalian (path, nodes_expanded)
    """
    open_set   = []
    heapq.heappush(open_set, (manhattan(start, goal), 0, start))
    came_from  = {}
